<script lang="ts">
	...
	import { searchWords, archives } from '$lib/stores/search';
	...
	const { categoryArchives } = $page.data;
	const limit = 20;
	const maxlength = 256;
	...
	function requestSearchByQuery() {
		...
		return SearchApi.getApiV1Search({
			...
			searchType: categoryArchives.type,
			limit: limit,
			offset: $archives.length
		})
			.then(data => {
				if (data.completedDistributionLiveDetails.length) {
					$archives = $archives.concat(data.completedDistributionLiveDetails);
					scroll?.validate();
				}
			})
			.catch(console.error);
...
<div class="p-4 py-3 md:(p-6 pr-8)">
	...
	{:else}
		<section class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 3xl:grid-cols-5 4xl:grid-cols-6 5xl:grid-cols-7 6xl:grid-cols-8 gap-4">
			{#each $archives as item (item.uuid)}
				<LiveCard
